...
function updateState() {
    ...
    if(alert) alert.classList.toggle("d-none", !isEmpty);
    if(clearBtn) clearBtn.classList.toggle("d-none", isEmpty);
    if(filterBtns) filterBtns.classList.toggle("d-none", isEmpty);
}
...
function handleFormSubmit(e) {
    ...
    if(input.value.trim().length === 0){
        alert("Lütfen yeni değer giriniz!");
        return;
    }
    addItem(input);
}
...
function filterItems(filterType) {
    ...
    for(let li of li_items){
        li.classList.remove("d-flex", "d-none", "d-block");
        const completed = li.hasAttribute("item-completed");
